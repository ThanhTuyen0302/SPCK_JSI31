...
// Count cart quantity
export async function countCartQuantity(email) {
  const cart = await getCartByEmail(email);
  const cartQty = cart.products.reduce((acc, item) => acc + item.qty, 0);
  return cartQty;
}

// sum cart total
export async function sumCartTotal(email) {
  const cart = await getCartByEmail(email);
  // dung id lam gia tien
  const total = cart.products.reduce(
    (acc, item) => acc + item.productId * item.qty,
    0
  );
  return total;
}
...
export async function setQty(email, productId, qty) {
  const cartRef = doc(firestore, "carts", email);
  const cart = await getCartByEmail(email);
  const products = cart.products || [];

  // Sanitize input
  const validatedQty = Math.max(1, Math.min(qty, 10)); // only 1–10 allowed
